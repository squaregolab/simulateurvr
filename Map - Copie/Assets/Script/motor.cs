...
public class motor : MonoBehaviour {
	...
	void Start()
	{
		...
		pot = Pot.Instance;
		sb = Sabertooth.Instance;
		cal = Calcul.Instance;

		try{
			string text = System.IO.File.ReadAllText ("setting.txt");
			power = float.Parse(text);
		//	Debug.Log(power.ToString() + " " + text);
		}
		catch{
			power = 15f;
		}
	...
	/// DifferenceAngle
	/// </summary>
	/// <returns>Differences entre les angles..</returns>
	/// <param name="AP">Angle potentiometre</param>
	/// <param name="AD">Angle desiree</param>
	float DifferenceAngle(int AP,float AD)
	{
		return AP - AD;
	}
	//Mise a 50% des moteurs
	////////////////////////////
	/// <summary>
	/// MoitieRi.
	/// </summary>
	/// <returns>Remise à 50% du moteur droit.</returns>
	/// <param name="potR">Pot r.</param>
	/*void moitieRi()
	{

		try 
		{
			int potR = int.Parse (pot.RiData);
			int ev =0;
			if (50< potR + 5 && 50 > potR - 5) 
				ev = 0;
			else if (potR <= 0)
				ev = -400;
			else if (potR >= 100)
				...
				ev = (int)((DifferenceAngle (potR, 50f)/100f*750f));
			SerialPort.WriteLine ("M2:" + ev.ToString ());
		}
		catch
		{
			Debug.Log ("Erreur parse Ri");
		}
	}
	/// <summary>
	/// MoitieLe.
	/// </summary>
	/// <returns>Remise à 50% du moteur gauche</returns>
	/// <param name="potL">Pot l.</param>
	void moitieLe ()
	{
		try 
		{
			int potL = int.Parse(pot.LeData);
			int ev =0;
			if (50< potL + 5 && 50 > potL - 5) 
				ev = 0;
			else if (potL <= 0)
				ev = -400;
			else if (potL >= 100)
				...
				ev = (int)((DifferenceAngle (potL, 50f)/100f*750f));
			}
			SerialPort.WriteLine ("M1:" + ev.ToString ());
		} catch {
			Debug.Log ("Erreur parse Le");
		}
	}
*/
	...
	private void GenericSerialLoop()
	{
		...
		try{
			axisX = cal.rotX;
			axisY = cal.rotY;
			//Debug.Log("V : " + axisV.ToString() + " Y : " + axisY.ToString());
			verti = (axisX < 2f || axisX > 2f) && axisX != 0f;
			hori = (axisY < 2f || axisY > 2f) && axisY != 0f;
		}
		catch{
			Debug.Log ("axis");
		}*/
			...
			try {
				...
				if (finR) {
					...
					try {
						...
						int angle = (int)((r-50));
						envoi = (int)((DifferenceAngle(angle,mtA)*power));			
						}
						//(int)((DifferenceAngle (potR, 50f)/100f*750f));
			...
			try {
				...
				if (finL) {
					...
					try {
						...
						{
						int angle = (int)((l-50));
						envoi = (int)((DifferenceAngle(angle,mtB)*power));
						}

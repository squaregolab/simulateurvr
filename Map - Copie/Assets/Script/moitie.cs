...
public class moitie : MonoBehaviour {
	public static moitie Instance;
		...
		private void GenericSerialLoop()
	{
			...
			//int l = int.Parse(pot.LeData);

			/*if(finL)
			{
				if(r>99)SerialPort.WriteLine("M2:-400");
				else if(r<1) SerialPort.WriteLine("M2:400");

				else if (50 > l + 20 && 50 < l - 20) {
						finL = false;
					}
				else if (l < 50) {
					SerialPort.WriteLine("M1:400");
				}	
				else if (l > 50) {
					SerialPort.WriteLine("M1:-400");
				}
			...
			else SerialPort.WriteLine ("M1:0");*/
		try {
			int ev =0;
			int r = int.Parse (pot.RiData);
			//Debug.Log("Ri : " +r);
			if (50< r + 1 && 50 > r - 1) 
				ev = 0;
			else if (r <= 0)
				...
				ev = (int)((DifferenceAngle (r, 50f)/100f*1500f));
			}
			SerialPort.WriteLine ("M2:" + ev.ToString ());
			Debug.Log("M2 : "+ev.ToString());
		} catch {
		...
		if (50< l + 1 && 50 > l - 1) 
			...
			ev = (int)((DifferenceAngle (l, 50f)/100f*1500f));
		}
		SerialPort.WriteLine ("M1:" + ev.ToString ());
		Debug.Log("M1 : "+ev.ToString());
	} catch {
		Debug.Log ("Erreur parse");
	}
	}
        ...
        lastacc = mgnt;
        //Acc inverse
        int InvAcc = Mathf.Clamp(100-acc,1,100);
        //Acceleration en %
        acc = Mathf.Clamp(acc, -1000, 1000);
        acc += 1000;
        acc /= 2000;
        acc *= 100;
        ////////////////////
        int nbStep = (minStep * (acc / 100));
        int i = 0;
        switch (tmotor.directionL)
        {
            case 0:
                SerialPort.WriteLine("M1:0");
                Thread.Sleep(10 * InvAcc);
                break;
            case 1:
                while (nbStep >= i && tmotor.directionL == 1)
                {
                    SerialPort.WriteLine("M1:0");
                    Thread.Sleep(1000);
                    SerialPort.WriteLine("M2:1000");
                    i++;
                }
                i = 0;
                break;
            case 2:
                while (nbStep >= i && tmotor.directionL == 2)
                {
